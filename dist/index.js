...
server.tool('generate-mind-map', GenerateMindMapSchema.shape, async (params) => {
    try {
        ...
        function buildTopic(topicData) {
            ...
            if (topicData.markers && topicData.markers.length > 0) {
                ...
                const markers = topicData.markers.map((markerStr) => {
            ...
            if (topicData.children && topicData.children.length > 0) {
                const children = topicData.children.map((child) => buildTopic(child));
        ...
        if (params.topics && params.topics.length > 0) {
            const children = params.topics.map((topic) => buildTopic(topic));
